...
parser.add_argument("-s", "--score", nargs=1, required=True, help="full path of score folder", metavar="PATH")
parser.add_argument("-o", "--summary", nargs=1, required=True, help="full path of summary folder", metavar="PATH")
...
for f in onlyfiles:
    ...
    protMerged=pd.merge(protAnnot,protDF, on='ORF Id', how='left')
    print("protMerged")
    print(protMerged.shape)
    ##Read Scoring file and merge.
    print("Reading:"+args.score[0]+sample+".scoreFunction4.tsv")
    protScore=readFile(args.score[0]+sample+".scoreFunction4.tsv",'\t')
    print("Dimension:")
    print(protScore.shape)
    print("protScore columnms:")
    print(protScore.columns.values)
    protScore['Decision']="Ref"
    protScore.loc[(protScore['Score']!=0) & (protScore['Score']>protScore['RefScore']),'Decision']="Iso"
    ##left outer join 
    protMerged['ORF Id'].replace(to_replace="\s+", value=' ', inplace=True, regex=True)
    protScore['ORF Id'].replace("\s+",' ', inplace=True, regex=True)
    protMergedScore=pd.merge(protMerged,protScore[['ORF Id','Score','RefScore','Decision']], on='ORF Id', how='outer')
    protMergedScore.Decision.fillna('Ref', inplace=True)
    ##Write protDF to a file
    print("protMerged 2:")
    print(protMerged.shape)
    print("proteinMat 1:")
    print(proteinMat.shape)
    print("score mat")
    print(protMergedScore.shape)
    proteinMat=proteinMat.append(protMergedScore)
    protMergedScore.drop('Class',axis=1, inplace=True)
    protMergedScore.rename(columns={'NewClass':'Class'},inplace=True)
    protMergedScore.to_csv(args.summary[0]+sample+".summary.tsv",sep='\t')
    print("proteinMat 2:")
    print(proteinMat.shape)
'''
...
unqPeptideEvdMat.to_csv(args.annot[0]+"/TGEs_pepEvd_score.tsv", sep="\t", index=False)
aggregations={
    'ALTPeptide':'sum',
    'UnqALTPeptide':'sum',
    'SALTPeptide':'sum',
...
pepEvdSwiss.to_csv(args.annot[0]+"/swiss_pepEvd_grpby_score.tsv", sep="\t")
pepEvdTrEMBL.to_csv(args.annot[0]+"/trembl_pepEvd_grpby_score.tsv", sep="\t")
print("Total Protein:")
...
print("Total number of TGEs with different types of variation/isoform Scoring Decision Variation")
if pepEvdSwiss.shape[0]>0:
    print("Swissprot novel Isoform: "+str(pepEvdSwiss[~(pepEvdSwiss['NewClass'].str.contains('known variation')) & (pepEvdSwiss['Decision'].str.contains('Iso'))].shape[0]))

if pepEvdTrEMBL.shape[0]>0:
    print("TrEMBL novel Isoform: "+str(pepEvdTrEMBL[~(pepEvdTrEMBL['NewClass'].str.contains('known variation')) & (pepEvdTrEMBL['Decision'].str.contains('Iso'))].shape[0]))

if pepEvdSwiss.shape[0]>0:
    print("Swissprot Known protein with Variations"+str(pepEvdSwiss[(pepEvdSwiss['NewClass'].str.contains('known variation')) & (pepEvdSwiss['Decision'].str.contains('Iso'))].shape[0]))

if pepEvdTrEMBL.shape[0]>0:
    print("TrEMBL Known protein with variations"+str(pepEvdTrEMBL[(pepEvdTrEMBL['NewClass'].str.contains('known variation')) & (pepEvdTrEMBL['Decision'].str.contains('Iso'))].shape[0]))
